...
# fck travado entre 10.0 MPa e 40.0 MPa conforme solicitado
...
# Taxa de argamassa limitada estritamente entre 45% e 60% em número inteiro
teor_argamassa_escolhido = st.sidebar.slider("Teor de Argamassa desejado (%)", min_value=45, max_value=60, value=53, step=1)

# Listas de opções oficiais de laboratório para as propriedades físicas (Ajustes de 10 em 10)
...
# Limites regulamentares de canteiro: umidade até 25% e inchamento até 50%
...
with col_fck:
    st.st.metric(label="fck Selecionado", value=f"{fck:.1f} MPa")
with col_fcj:
    st.st.metric(label="fcj 28 Dias Calculado", value=f"{fcj:.1f} MPa", delta="sd = 4.0")

st.markdown("---")

# -----------------------------------------------------------------------------
# MOTOR DE DOSAGEM INTELIGENTE - EQUIVALENTE ÀS NOVAS ABAS ENVIADAS
# -----------------------------------------------------------------------------
def buscar_e_calcular_dosagem(tipo_cimento):
    # Consumo de água padrão cruzando slump e dmax (Conforme as abas do novo arquivo)
    ...
    # CORREÇÃO: Vinculação matemática inversa corrigida conforme as tabelas das abas novas
    # O cimento CP II-32 possui uma relação a/c menor, gerando consumo MAIOR que o CP II-40
    if tipo_cimento == "CP II 32":
        ac = 0.27 + ((46.6 - fcj) * 0.013)
        ac = max(0.20, min(0.68, ac))
    else:
        ac = 0.39 + ((46.6 - fcj) * 0.009)
        ac = max(0.30, min(0.80, ac))

    # Cálculo encadeado: a/c comanda o cimento, que comanda o volume de agregados
    cc = ca_inicial / ac
    c_adit = cc * 0.007
    
    # Busca do coeficiente Vb baseado no diâmetro do agregado graúdo
    vb = 0.565 if dmax_escolhido == 9.5 else (0.622 if dmax_escolhido == 12.5 else 0.690)
    cb_total = vb * mu_brita
    
    # Cubagem e exclusão de volumes absolutos (MF = 2.6 e Ar = 2% rodando fixos)
    ...
    # Efeito ativo de Umidade: adiciona a água na massa da areia e desconta da água limpa
    ...
    # Distribuição proporcional configurada nas barras de agregados
    ...
    # Efeito ativo de Inchamento: altera diretamente o volume de obra calculado na Seção 2
